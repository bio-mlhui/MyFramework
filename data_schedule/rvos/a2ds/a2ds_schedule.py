...
from data_schedule.rvos.video_refer_aug.augmentation_videos import video_aug_entrypoints
...
from data_schedule.utils.sampler import Train_InfiniteSampler_Distributed, Evaluate_Sampler_Distributed
...
from data_schedule.utils.dataset_auxiliary import CollatorWithAux, DatasetWithAux
...
# 和generate params没关系
def evaluate(loader, model, device, is_distributed, is_main_process, output_dir, validate_metrics, root):
    eval_metrics = {}
    if 'pFpE_mAP_Pat_IOU' in validate_metrics:
        ...
        for idx, batch_dict in tqdm(enumerate(loader)):
            ...
            model_preds = model.sample(batch_dict, visualize_dir=None)
...
def a2ds_schedule(configs, is_distributed, process_id, num_processes):
    ...
    # dataloader part
    sampler_train = Train_InfiniteSampler_Distributed(train_dataset,
    ...
    self.validateset_vis_idxs = configs['visualize']['validateset_idxs']
    self.testset_vis_idxs = configs['visualize']['testset_idxs']

    return train_loader, sampler_train,\
           partial(evaluate, loader=validate_loader, validate_metrics=validate_metrics, root=root)
...
class Collator(CollatorWithAux):
    ...
    def batching_targets(targets):
        ret = {}
        if 'masks' in targets[0]:
            ret['masks'] = [t['masks'] for t in targets]
        if 'class_labels' in targets[0]:
            ret['class_labels'] = [t['class_labels'] for t in targets]
        if 'boxes' in targets[0]:
            ret['boxes'] = [t['boxes'] for t in targets]
        if 'valid' in targets[0]:
            ret['valid'] = [t['valid'] for t in targets]
        if 'referent_idx' in targets[0]:
            ret['referent_idx'] = [t['referent_idx'] for t in targets]
        if 'orig_size' in targets[0]:
            ret['orig_size'] = [t['orig_size'] for t in targets]
        if 'size' in targets[0]:
            ret['size'] = [t['size'] for t in targets]        
        if 'has_ann' in targets[0]:
            ret['has_ann'] = [t['has_ann'] for t in targets]
        return ret
    ...
    def __call__(self, batch):
        ...
        if self.split == 'test':
            ...
            batch_data['targets'] = self.batching_targets(meta_or_target)
