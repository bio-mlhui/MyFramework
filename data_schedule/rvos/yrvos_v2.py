...
class YRVOS_Dataset(DatasetWithAux):      
    ...
    def __getitem__(self, sample_idx):
        if self.split == 'validate' or self.split == 'test':
            video_id, window_frames, exp_id = self.samples[sample_idx] 
            window_frames = sorted(window_frames)
                       
            all_exps_dict = self.text_annotations_by_videoid[video_id]["expressions"]
            text_query = all_exps_dict[exp_id]['exp']
            frame_dir = 'train/JPEGImages' if self.split == 'validate' else 'valid/JPEGImages'
            vframes = [Image.open(os.path.join(self.root, frame_dir, video_id, f'{f}.jpg')) for f in window_frames]
            has_ann = torch.ones([len(vframes)]).bool()
            image_ids = []
            for frame in window_frames:
                image_ids.append(f'v_{video_id}_f{frame}_e_{exp_id}')
            ...
            return asembles, meta,  self.get_aux(sample_idx)
        elif self.split == 'train':
            video_id, window_frames, exp_id = self.samples[sample_idx]            
            all_exps_dict = self.text_annotations_by_videoid[video_id]["expressions"]
            all_objs_dict = self.obj_annotations_by_videoid[video_id]["objects"]
            text_query = all_exps_dict[exp_id]['exp']
            vframes = [Image.open(os.path.join(self.root, 'train/JPEGImages', video_id, f'{f}.jpg')) for f in window_frames]
            ...
            return vframes, text_query, self.get_aux(sample_idx, exist_queries=annotated_exps_by_object,
                                                     video_auxid=None, text_auxid=None), targets
