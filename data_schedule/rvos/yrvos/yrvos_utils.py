...
def check_yrvos_validity(root): 
    ...
    assert len(list(YRVOS_CATEGORIES.keys())) == 65
    assert len(set(list(YRVOS_CATEGORIES.values())) - set(list(range(65)))) == 0
    assert len(set(list(range(65))) - set(list(YRVOS_CATEGORIES.values()))) == 0
    assert len(set(list(YRVOS_TRAIN_VIDEO_IDS))) == 3471
    assert len(set(list(YRVOS_TEST_VIDEO_IDS))) == 202
...
YRVOS_TRAIN_VIDEO_IDS = np.array(YRVOS_TRAIN_VIDEO_IDS)

YRVOS_TEST_VIDEO_IDS = [
    "547416bda1",
    "ab9a7583f1",
    "0c04834d61",
    "ee9415c553",
...
# Sliding Window deterministic samples
...
def generate_samples_of_one_video(root, video_annotations, video_id, split, generate_params):
    ...
    if split == 'train':
        ...
        sampled_windows = generate_windows_of_video(all_frames, 
