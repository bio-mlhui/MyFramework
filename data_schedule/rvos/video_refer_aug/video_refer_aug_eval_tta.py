...
_video_refer_eval_aug_tta_entrypoints = {}

def register_video_refer_eval_aug_tta(fn):
    aug_name = fn.__name__
    _video_refer_eval_aug_tta_entrypoints[aug_name] = fn

    return fn


def video_refer_eval_aug_tta_entrypoints(aug_name):
    try:
        return _video_refer_eval_aug_tta_entrypoints[aug_name]
...
# TODO谁是size_orig, 谁是size
def resize_callback(rescaled_video, texts, preds, size):
    video = [F.resize(frame, size) for frame in rescaled_video]
    
    ratios = tuple(float(s) / float(s_orig) for s, s_orig in zip(video[0].size, rescaled_video[0].size))
...
@register_video_refer_eval_aug_tta
