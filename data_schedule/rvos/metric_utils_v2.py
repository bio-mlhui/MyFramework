...
def eval_function(coco_gt: COCO, coco_pred: COCO, instance):
...
def calculate_precision_at_k_and_iou_metrics(coco_gt: COCO, coco_pred: COCO):
    ...
    params = [(coco_gt, coco_pred, instance) for instance in coco_gt.imgs.keys()]
    n_jobs = min(multiprocessing.cpu_count(), 12)
    res_list = Parallel(n_jobs)(delayed(eval_function)(*p) for p in tqdm(params))
    res_list = [s for l in res_list for s in l] 
    
    intersec_list, union_list, ious_list  = list(zip(*res_list))
