...
class Train_InfiniteSampler_Distributed(Sampler[T_co]):
    # 每次for loader时, __iter__被调用, 返回一个infinite sample stream
    # 不同Process之间的seed相同
    # 通过set_iter_first_sample_idx设置下一次for loader的第一个sample的下标
    # make_epoch_clear
    def __init__(self, 
                 inf_stream_fn, # 保证每个sampler的infinite stream都一样
                 start_idx: int = 0, # 调用iter返回的第一个sample的下标
                 end_idx = None,
    ...
    def set_iter_first_sample_idx(self, idx):
        # idx 调用iter方法yield的第一个sample的idx
        self.start_idx = idx
    ...
    # 每个loader 一个sampler, 每个sampler都有一样的infinite stream
    # 每个loader的sampler 但有不一样的start_idx, end_idx, 移花接木
    # 只
    ...
    #     while True:
    #         random_permutations = torch.randperm(len(self.dataset), generator=g).tolist()
    #         if self.make_epoch_clear:
    #             if len(random_permutations) % self.num_replicas != 0:
    #                 padding = self.num_replicas - (len(random_permutations) % self.num_replicas)
    #                 padding = torch.randperm(len(self.dataset), generator=g)[:padding].tolist()
    #             random_permutations = random_permutations + padding
    #             assert len(random_permutations) % self.num_replicas == 0
