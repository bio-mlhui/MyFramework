...
import gc
import json
from .utils import get_data_dir, parse_image_data, parse_region_descriptions, parse_QA
...
def get_all_image_data(data_dir=None):
    ...
    return [parse_image_data(image) for image in data]
...
def get_all_region_descriptions(data_dir=None):
    ...
    output = []
    for image in images:
        output.append(parse_region_descriptions(
...
def get_all_qas(data_dir=None):
    ...
    output = []
    for image in images:
        output.append(parse_QA(image['qas'], image_map))
    return output
...
def save_scene_graphs_by_id(data_dir='data/', image_data_dir='data/by-id/'):
    ...
    Each output .json has the following keys:
      - "id"  
      - "objects": 40,   list[{'synsets': ['clock.n.01'], 'h': 339, 'object_id': 1058498, 'names': ['clock'], 'w': 79, 'attributes': ['green', 'tall'], 'y': 91, 'x': 421}]   {'synsets': ['next.r.01'], 'predicate': 'next to', 'relationship_id': 15933, 'object_id': 1058539, 'subject_id': 1058534}
      - "relationships": 41, list[{"synsets':['along.r.o1'], 'predicate':'ON', 'relationship_id', 'object_id', 'subject_id'}]
    """
    ...
    for sg_data in tqdm(all_data): # {'relationships', 'image_id', 'objects'}
        saved_dir = os.path.join(image_data_dir, str(sg_data['image_id']))
        os.mkdir(saved_dir)
        with open(os.path.join(saved_dir, 'scene_graph.json'), 'w') as f:
            json.dump(sg_data, f)
...
def save_region_graphs_by_id(data_dir='data/', image_data_dir='data/by-id/'):
    """
    Save a separate .json file for each regino id in `image_data_dir`.
    ...
    with open(os.path.join(data_dir, 'region_graphs.json'), 'r') as f:
        all_data = json.load(f)

    # list["regions":list[dict], "id":image_id]
    for sg_data in tqdm(all_data): 
        regions = sg_data['regions'] # 262 list[{'relationships': [], 'region_id': 1382, 'width': 82, 'synsets': [{...}, {...}], 'height': 139, 'image_id': 1, 'objects': [{...}], 'phrase': 'the clock is green in colour', 'y': 57, 'x': 421}]
        img_id = sg_data['image_id']
        
        region_id_key = 'region_id'
        
        by_id_img_id_dir = os.path.join(image_data_dir, str(img_id))
        if not os.path.exists(by_id_img_id_dir):
            os.mkdir(by_id_img_id_dir)
            
        for reg in regions:
            reg_file_name = 'region' + str(reg[region_id_key]) + '.json'
            with open(os.path.join(by_id_img_id_dir, reg_file_name), 'w') as f:
                json.dump(reg, f)
