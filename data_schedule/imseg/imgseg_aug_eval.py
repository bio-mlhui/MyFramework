...
_imgseg_evalaug_entrypoints = {}

def register_imgseg_evalaug(fn):
    aug_name = fn.__name__
    _imgseg_evalaug_entrypoints[aug_name] = fn

    return fn


def imgseg_evalaug_entrypoints(aug_name):
    try:
        return _imgseg_evalaug_entrypoints[aug_name]
...
class Resize(object):
    ...
    def __call__(self, img, meta):
        ...
        tgt_size = get_tgt_size(img.size, self.min_size, max_size=self.max_size)
        resized_img = resize(copy.deepcopy(img), tgt_size)
        asembles.append([resized_img, meta, [partial(resize_callback, size=[orig_h, orig_w])]])
        return asembles
    
@register_imgseg_evalaug
def resize(configs):
    return Resize(min_size=configs['min_size'],
                  max_size=configs['max_size'])

@register_imgseg_evalaug
def resizes_hflip_asemble(configs):
    return RandomResize_HFlip_Asemble(sizes=configs['min_sizes'],
