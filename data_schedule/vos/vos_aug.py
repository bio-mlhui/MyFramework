...
对于model-aware augmentation, 需要加到aux_mapper里, 要注意aux_mapper只能添加dict, 不能改变video_dict

VOS_aug公共接口
{'video', 'masks', 'boxes, 'callback_fns', 'has_ann', 'class_labels',}
...
VOS_AUG_REGISTRY = Registry('VOS_AUG')
    ...
    def __call__(self, old_ret):
        ...
        if random.random() < self.p:
            ...
            if "pred_masks" in ret:
                # t'(train)/t(eval_callback) h w, bool
...
class _RandomResize:
    ...
    def __call__(self, old_ret):
        ...
        if "masks" in ret:
            ...
            masks = ret['masks']
            # nearest 不会改变0-1值
            masks = torch.nn.functional.interpolate(masks.unsqueeze(0).float(), tgt_size, mode='nearest', align_corners=None)[0].bool()
            ret['masks'] = masks
        
        if "pred_masks" in ret:
            # nq t h w, float
            pred_masks = ret['pred_masks']
            assert pred_masks.dtype == torch.float
            if len(pred_masks.shape) == 3:
                pred_masks = torch.nn.functional.interpolate(pred_masks.unsqueeze(0), tgt_size, mode='bilinear', align_corners=True)[0]
            elif len(pred_masks.shape) == 4:
