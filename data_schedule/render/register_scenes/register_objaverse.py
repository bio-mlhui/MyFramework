...
def get_rendering_fn(view_camera=None, return_alpha=True, **kwargs):
...
def get_extrinstic_fn(view_camera=None, cam_radius=None, **kwargs):
    # list[dict, view_camera:str, rendering_path:str]
...
def get_intrinstic_fn(view_camers=None, **kwargs):
    pass


def objaverser_meta(ov_root, num_views, start_angle=36, end_angle=73,  seed=None, split_dataset_name=None):
    angles = torch.arange(start=start_angle, end=end_angle)
    scene_files = []
    ...
    Scene_Meta
    metas = []
    for scene_uid in tqdm(scene_files):
        vids = angles[torch.randperm(len(angles), generator=g)[:num_views]].tolist() + torch.randperm(100, generator=g).tolist()
        view_cameras = []
        for vid in vids:
            image_path = os.path.join(scene_uid, 'rgb', f'{vid:03d}.png')
            camera_path = os.path.join(scene_uid, 'pose', f'{vid:03d}.txt')
            assert os.path.exists(image_path) and os.path.exists(camera_path)
            view_cameras.append({'view_camera': camera_path, 'rendering_path': image_path})
        metas.append({
            'scene_id': scene_uid,
            'metalog_name': split_dataset_name,
