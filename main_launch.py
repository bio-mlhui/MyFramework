...
def run(rank, configs, world_size):
    ...
    if configs['trainer_mode'] == 'eval':
        ...
        for ckpt in eval_ckpts:
            trainer.load_ckpt(ckpt, load_model=True, load_schedule=True, load_optimize=False)
            trainer.evaluate()
            # 计算Metrics
            # 存储metrics到ckpt
        ...
        if configs['trainer_mode'] == 'train_resume':
            ...
            trainer.load_ckpt(trainer_ckpt, load_model=True, load_schedule=True, load_optimize=True)
        trainer.train()
...
if __name__=="__main__":
    ...
    task, group, config, _ = args.config_file.split('/')[-4:]
    ...
    # num_gpus_per_machine = len(gpu_ids)
