...
@META_ARCH_REGISTRY.register()
class BHWC_Masked_Deformable_SS_Func(nn.Module):
    def __init__(self, 
                 configs, ):
        super().__init__()
        d_model = configs['d_model']
        n_levels = configs['n_levels']
        n_scan_dircs = configs['n_scan_dircs']
        n_points = configs['n_points'] # K
    ...
    def forward(self, 
                ...
                reference_points,  # b Nq L 2; 每个query point在每个scale的位置
        ...
        """
        B, Nq, _ = query_seq.shape
        B, HW_sigma, _ = input_flatten.shape
        assert (input_spatial_shapes[:, 0] * input_spatial_shapes[:, 1]).sum() == HW_sigma

        # b Nq c -> b Nq n_scan_dircs L K 2; H/W的相对delta
        sampling_offsets = self.sampling_offsets(query_seq).view(B, Nq, self.n_scan_dircs, self.n_levels, self.n_points, 2)
        ...
        # b hw_sigma head c -> list[b hw head c], L
        input_flatten = input_flatten.split([H_ * W_ for H_, W_ in input_spatial_shapes], dim=1)
