...
from .utils import find_scale_from_multiscales, find_scales_from_multiscales
...
class DeformableTransformer(nn.Module):
    ...
    def get_valid_ratio(self, mask):
        ...
        Input:
            - mask: T(b h w), bool
        Output: 
            - valid_ratio of every image: T(b 2)
        """
        _, H, W = mask.shape
    ...
    def forward(self, srcs, tgt, masks, pos_embeds, query_embed=None):
        ...
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=src_flatten.device)  # level 2
        level_start_index = torch.cat((spatial_shapes.new_zeros((1, )), spatial_shapes.prod(1).cumsum(0)[:-1])) # [0, level_1_start, level2_start, ], level
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1)  # T( b,l, 2), [0,1]
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None,):
        """
        src: TB (h_sigma, w_sigma)  c
        text_word: T(s b c)
        """
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        sampling_locations_by_layer = []
        attention_weights_by_layer = []
        for idx, layer in enumerate(self.layers):
            output, sampling_locations, attention_weights = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask)           
            sampling_locations_by_layer.append(sampling_locations)
            attention_weights_by_layer.append(attention_weights)
        return output, sampling_locations_by_layer, attention_weights_by_layer
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        ...
        Input:
            ...
            - query_pos:
                T(n b c)
            - reference_points: absolute coordinate
                T(n b 2/4)
            - src:
                T((h_sigma, w_sigma) b c)
            - src_spatial_shapes:
                T(level 2), absolute
            - level_start_index:
                [0, level1_start, level2_start, ...]
        """
        # self attention
        ...
        # cross attention
        # T(n b c)
        # T(n b M*L_2K), absolute
        # T(n b M*L_K), sum to one for each query
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        """
        Input: 
            - tgt: text_sentence level features
                T(b n c)
            - reference_points: normalized position (x,y) w.r.t the single feature map, [0, 1]
                T(b n 2)
            - src:
                T(b h_sigma w_sigma c)
            - src_spatial_shapes:
                T(level 2), absolute
            - src_level_start_index:
                [0, level1_start_idx, ..., ], level
            - src_valid_ratios: valid_h, valid_w [0, 1] of each image in the batch
                T(b level 2),
            - query_pos:
                T(b n c)
        """
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                reference_points_input = reference_points[:, :, None] \
                                         * torch.cat([src_valid_ratios, src_valid_ratios], -1)[:, None]  # b n 1 4 * b 1 level 4
            else:
                assert reference_points.shape[-1] == 2
                # normalized -> absolute
                reference_points_input = reference_points[:, :, None] * src_valid_ratios[:, None]  # b n 1 2 * b 1 level 2 -> b n level 2
            # T(b n c)
            # T(b n M L K 2), abolute w.r.t each level
            # T(b n M L K)
            ...
            sampling_locations = sampling_locations / src_valid_ratios[:, None, None, :, None, :] # / b 1 1 level 1 2
