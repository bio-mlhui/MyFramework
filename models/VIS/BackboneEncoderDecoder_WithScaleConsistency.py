...
class BackboneEncoderDecoder_WithScaleConsistency(nn.Module):
    ...
    #     # plt.imsave('./mask.png', mask[0][0].cpu().numpy())
    #     prediction = self.model_preds(videos, video_aux_dict=batch_dict['video_dict']) # {pred_masks: b 1 t h w}
    #     dec_loss = self.decoder.compute_loss(prediction, targets=targets, frame_targets=batch_dict['frame_targets'],
    #                                            video_aux_dict=batch_dict['video_dict'])

    #     loss_value_dict = {key: dec_loss[key] for key in list(self.loss_weight.keys())}
    #     # gradient_norm = get_total_grad_norm(self.model.parameters(), norm_type=2)
    #     return loss_value_dict, self.loss_weight
    ...
    def get_optim_params_group(model, configs):
        ...
        for module_name, module in model.named_modules():
            for module_param_name, value in module.named_parameters(recurse=False):
                ...
                hyperparams = copy.copy(defaults)
                if 'segformer_head.linear_c' in module_name or 'video_backbone.outnorm' in module_name or 'segformer_head.classifier' in module_name:
                    hyperparams["lr"] = configs['optim']['base_lr']  
                    hyperparams["weight_decay"]  = configs['optim']['base_wd'] 
                    if log_lr_group_idx['base'] is None:
                        log_lr_group_idx['base'] = len(params)
