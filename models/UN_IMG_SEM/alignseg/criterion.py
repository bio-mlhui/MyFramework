...
class AlignCriterion(nn.Module):
    ...
    def forward(self, results):
        ...
        gc_token_assign = torch.einsum("bnc,bqc->bnq", norm(gc_output), norm(all_queries[0])) # b hw nq
        ...
        for i in range(self.nmb_crops[-1]):
            ...
            # b hw nq, b hw_local nq -> b hw hw_local
            # b hw c, b hw_local c -> b hw hw_local
        ...
        return loss, {'gclc_cor_loss': gclc_cor_loss.item() / self.nmb_crops[-1],
                      'query_align_loss': query_align_loss.item() / self.nmb_crops[-1] }
