...
import sys
from .code import aggo_merge, aggo_merge_graph, aggo_whole_batch
...
def visualize_cutler_onlyAttn(image, cluster_attn, patch_size, lengths, image_path, image_name):
    ...
    #         out_features.extend(features.chunk(end_idx-start_idx, dim=0))
    #         start_idx = end_idx        
    #     return out_features # list[b hi wi c]

    # def forward_backward_v0(self, batch_dict):
    #     current_epoch = batch_dict['num_epoch']
    #     device = self.device
    #     assert self.training
    #     trainingiter = batch_dict['num_iterations']
    ...
    #     images = [foo.to(device, non_blocking=True) for foo in images]
    #     batch_size = images[0].shape[0] 
    #     # list[dict: grid_sample], batch
    #     common_samples = batch_dict['common_samples'] 
    #     # list[b hi wi c], crop
    #     image_features = self.forward_backbone_features(images)
    ...
    #         for batch_idx in range(batch_size):
    #             cluster_idx = crop_idx * batch_size + batch_idx
    #             clu_feats = cluster_feats[cluster_idx] # list[ni c], threshold
    #             clu_lengths = [len(foo) for foo in clu_feats]
    #             clu_feats = torch.cat(clu_feats, dim=0)
    #             cluster_attn = torch.einsum('hwc,nc->hwn', 
    #                                         F.normalize(image_features[crop_idx][batch_idx], dim=-1, eps=1e-10), 
    #                                         F.normalize(clu_feats, dim=-1, eps=1e-10))
    #             visualize_cutler_onlyAttn(images[crop_idx][batch_idx], cluster_attn=cluster_attn, patch_size=self.patch_size,
    #                                       lengths=clu_lengths, image_path=f'./temp', image_name=f'{crop_idx}_{batch_idx}.png')
        
    #     # list[b hi wi k]
    #     teacher_crops = [] 
    #     student_crops = []
    #     teacher_nqs = [] # list[list[ni c], batch] crop
    #     with torch.cuda.amp.autocast(self.fp16_scaler is not None):
    #         for crop_idx in range(num_crops):
    #             if crop_idx < 2:
    #                 hw_Ks_teacher = [] #list[h w k], batch
    #                 nq_teacher = [] # list[ni c], batch
    #             hw_Ks_student = []
    #             for batch_idx in range(batch_size):
    #                 with torch.no_grad():
    #                     batch_crop_feat = image_features[crop_idx][batch_idx] # h w c
    #                     # nq c
    #                     # cluster_masks, cluster_feats, lengths = aggo_merge(batch_crop_feat, threshes=self.merge_thresholds, min_size=self.merge_minsize)
    #                     cluster_masks, cluster_feats, lengths = aggo_merge_graph(batch_crop_feat)
    #                     # h w nq 相似度, -1, 1
    #                     cluster_attn = torch.einsum('hwc,nc->hwn', 
    #                                                   F.normalize(batch_crop_feat, dim=-1, eps=1e-10), 
    #                                                   F.normalize(cluster_feats, dim=-1, eps=1e-10))
    #                     visualize_cutler_onlyAttn(images[crop_idx][batch_idx], cluster_attn, self.patch_size, lengths)
    #                     cluster_attn = F.softmax(cluster_attn / self.nq_temperature, dim=-1)
    #                 if crop_idx < 2:  
    #                     teacher_nq = self.teacher_head(cluster_feats)
    #                     hw_Ks_teacher.append(torch.einsum('hwn,nk->hwk', cluster_attn, teacher_nq))
    #                     nq_teacher.append(teacher_nq)
    #                 student_nq = self.student_head(cluster_feats)
    #                 hw_Ks_student.append(torch.einsum('hwn,nk->hwk', cluster_attn, student_nq))

    #             if crop_idx < 2:
    #                 teacher_crops.append(torch.stack(hw_Ks_teacher, dim=0)) 
    #                 teacher_nqs.append(nq_teacher)
    #             student_crops.append(torch.stack(hw_Ks_student, dim=0))
            
    #         loss = self.dino_loss(student_output=student_crops, teacher_output=teacher_crops, 
    #                               epoch=current_epoch,
    #                               common_samples=common_samples,
    #                               teacher_nqs=teacher_nqs)
    ...
    #         if self.optim_configs['clip_grad']:
    #             param_norms = utils.clip_gradients(self.student_head, self.optim_configs['clip_grad'])
    #         utils.cancel_gradients_last_layer(epoch=current_epoch, 
    #                                           model=self.student_head,
    #                                           freeze_last_layer=self.optim_configs['freeze_last_layer'])
    #         self.optimizer.step()
    #     else:
    #         self.fp16_scaler.scale(loss).backward()
    #         if self.optim_configs['clip_grad']:
    #             self.fp16_scaler.unscale_(self.optimizer)  # unscale the gradients of optimizer's assigned params in-place
    #             param_norms = utils.clip_gradients(self.student_head, self.optim_configs['clip_grad'])
    #         utils.cancel_gradients_last_layer(epoch=current_epoch, model=self.student_head,
    #                                           freeze_last_layer=self.optim_configs['freeze_last_layer'])
    #         self.fp16_scaler.step(self.optimizer)
    #         self.fp16_scaler.update()
