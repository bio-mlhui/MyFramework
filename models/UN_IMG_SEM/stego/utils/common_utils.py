...
from .wandb_utils import set_wandb, set_logging_file
...
def parse(json_path: str, is_debug, is_test) -> dict:
    if json_path[-5:] == '.json':
        with open(json_path, "r", encoding="utf-8") as f:
            opt = json.load(f, object_pairs_hook=OrderedDict)  # noqa
    elif json_path[-3:] == '.py':
        file = json_path.split('/')[-1][:-3]
        import importlib
        opt = importlib.import_module(f'configs.server.{file}').config
    elif json_path[-4:] == '.yml':
        import yaml
        with open(json_path, 'r') as file:
            opt = yaml.safe_load(file.read()) 
    else:
        raise ValueError()       
    ...
    opt["save_dir"] =  os.path.join(opt['output_dir'], f'{opt["wandb"]["group"]}_{opt["wandb"]["name"]}')
    os.makedirs(opt['save_dir'], exist_ok=True)
    local_rank = 0
    wandb_save_dir = set_wandb(opt, local_rank, force_mode="disabled" if (is_debug or is_test) else None)
    
    opt['save_dir'] = wandb_save_dir
    opt["full_name"] = opt['save_dir']
    with open(opt["save_dir"] + '/option.json', 'w', encoding='utf-8') as f:
        json.dump(opt, f, indent="\t")

    set_logging_file(opt['save_dir'], file_name='test.log' if is_test else 'train.log', mode='w')
    return opt
