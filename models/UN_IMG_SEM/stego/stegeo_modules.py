...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def helper(self, f1, f2, c1, c2, shift):
        ...
        if self.config['zero_clamp']:
            min_val = 0.0
        else:
            min_val = -9999.0

        if self.config['stabalize']:
    ...
    def forward(self,
        ...
        coord_shape = [orig_feats.shape[0], self.config['feature_samples'], self.config['feature_samples'], 2]
        ...
        pos_intra_loss, pos_intra_cd = self.helper(
            feats, feats, code, code, self.config['pos_intra_shift'])
        pos_inter_loss, pos_inter_cd = self.helper(
            feats, feats_pos, code, code_pos, self.config['pos_inter_shift'])

        neg_losses = []
        neg_cds = []
        for i in range(self.config['neg_samples']):
            perm_neg = super_perm(orig_feats.shape[0], orig_feats.device)
            feats_neg = sample(orig_feats[perm_neg], coords2)
            code_neg = sample(orig_code[perm_neg], coords2)
            neg_inter_loss, neg_inter_cd = self.helper(
                feats, feats_neg, code, code_neg, self.config['neg_inter_shift'])
