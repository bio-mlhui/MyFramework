...
class Downsampling_REG(nn.Module):
    """
    Downsampling implemented by a layer of convolution.
    """
    def __init__(self, in_channels, out_channels, 
...
class Attention_REG(nn.Module):
    def __init__(self, reg_cls,
                 dim, head_dim=32, num_heads=None, qkv_bias=False,
        attn_drop=0., proj_drop=0., proj_bias=False, **kwargs):
        super().__init__()
        self.reg_cls = [reg_cls] # do not contain in module, very hacky
        self.head_dim = head_dim
        self.scale = head_dim ** -0.5
...
class MetaFormerBlock(nn.Module):
    ...
    def __init__(self, dim,
        ...
        super().__init__()
        assert layer_scale_init_value is None
        self.norm1 = norm_layer(dim)
        self.is_attention = token_mixer == Attention
    ...
    def forward(self, x):
        res_x = self.res_scale1(x)
        trans_x = self.layer_scale1(
                self.drop_path1(
...
DOWNSAMPLE_LAYERS_FOUR_STAGES_LAST_REG = [partial(Downsampling,
            kernel_size=7, stride=4, padding=2,
            post_norm=partial(LayerNormGeneral, bias=False, eps=1e-6)
            )] + \
            [partial(Downsampling,
...
class MetaFormer_BuildCls:
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        self.downsample_layers = \
            [downsample_layers[i](down_dims[i], down_dims[i+1]) if i != dino_stage_idx else None for i in range(num_stage)]
        ...
        self.stages = [] # each stage consists of multiple metaformer blocks
        cur = 0
