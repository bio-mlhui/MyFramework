...
try:
    ...
    from .hilbert_sca import Hilbert2D
except:
    from csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            ...
            v5=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, CrossScan=Hilbert2D(), CrossMerge=Hilbert2D()),
        )
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.ssm_branch:
            ...
            temporal_block_configs.update(
                {
                    'd_model': hidden_dim,
                }
            )            
            self.train_batch_size = temporal_block_configs['train_batch_size'] // comm.get_world_size()
    ...
    def _forward(self, input: torch.Tensor):
        if self.ssm_branch: 
            # time first
            b_t, H, W, D = input.shape
            nf = b_t // self.train_batch_size
            input = rearrange(input, '(b t) h w c -> (b h w) t c',b=self.train_batch_size)
            # temporal_pos = self.pos_1d(torch.zeros_like(input[:, :, 0]).bool(), hidden_dim=input.shape[-1]).permute(0, 2, 1) # b s c
            # input += temporal_pos
            batch_size = self.train_batch_size if self.training else 1

            if self.post_norm:
                input = input + self.drop_path(self.temporal_norm(self.temporal_block(input, batch_size=self.train_batch_size)))
                input = rearrange(input, '(b h w) t c -> (b t) h w c',h=H, w=W,t=nf)
                x = input + self.drop_path(self.norm(self.op(input)))
            else:
                input = input + self.drop_path(self.temporal_block(self.temporal_norm(input), batch_size=self.train_batch_size))
                input = rearrange(input, '(b h w) t c -> (b t) h w c',h=H, w=W,t=nf)
...
class SplitTemporal_VMamba(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for name, feat in zip(names, outs):
            ret[name] = rearrange(feat, '(b t) c h w -> b c t h w',b=batch_size)
        return ret
