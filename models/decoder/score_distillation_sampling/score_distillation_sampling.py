...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, sd_version='2.1', hf_key=None, t_range=[0.02, 0.98]):
        ...
        logging.debug(f'[INFO] loading stable diffusion...')
        if hf_key is not None:
            logging.debug(f'[INFO] using hugging face custom model key: {hf_key}')
            model_key = hf_key
        ...
        # Create model
        pipe = StableDiffusionPipeline.from_pretrained(os.path.join(os.getenv('PT_PATH'), 'stable_diffusion', model_key), torch_dtype=self.precision_t)
        if vram_O:
        ...
        logging.debug(f'[INFO] loaded stable diffusion!')
