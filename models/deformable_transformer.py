...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, 
                text_word=None, text_padding_mask=None, text_pos=None):
        """
        src: TB (h_sigma, w_sigma)  c
        text_word: T(s b c)
        """
        output = src
        batch_size = text_word.shape[1]
        nf = src.shape[0] // batch_size
        hw = src.shape[1]
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for idx, layer in enumerate(self.layers):
            output = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask) 
    
            if (idx != len(self.layers) - 1) and (self.fusion_module is not None):
                
                output = rearrange(output, '(t b) hw c -> (t hw) b c',t=nf, b=batch_size)
                output =  self.fusion_module(tgt=output,
                            memory=text_word,
                            memory_key_padding_mask=text_padding_mask,
                            pos=text_pos,
                            query_pos=None)
                output = rearrange(output, '(t hw) b c -> (t b) hw c',t=nf,hw=hw)
