...
class VSSBlock_v0(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = Mamba(d_model=hidden_dim, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path)
...
class VSSLayer_3d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.flatten(2).permute(0, 2, 1).contiguous() # b thw c
        for blk in self.blocks:
            x = blk(x)
        
        x = rearrange(x, 'b (t h w) c -> b c t h w',t=nf, h=H, w=W)
