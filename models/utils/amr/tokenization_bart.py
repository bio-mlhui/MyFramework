...
import models.utils.amr.common.postprocessing as postprocessing
...
class AMRBartTokenizer(BartTokenizer):
    ...
    def init_amr_vocabulary(self):
        ...
        # additional-tokens.json, 一堆看似是ontonotes上的predicates

        for i, t in enumerate(tokens, start=old_enc_size):
            self.encoder[t] = i
                
        # vocab.json (free-form English words + some predicates) + additional-predicates
        ...
        logging.debug(f"Added {self.modified} AMR tokens")
    ...
    def tokenize_amr(self, amr_tokens):
        """
        amr_tokens: ( <pointer:0> limp-01~3 :ARG0 ( <pointer:1> pigeon~1 ) )
        """
        bpe_tokens = [] # bpe_token 每个元素是list['G', ..] 即一个token的subwords
        ...
        for i, tokk in enumerate(amr_tokens):
            ...
            is_in_enc = self.INIT + tokk in self.encoder   # Ġ<pointer:0>
            ...
            if tokk.startswith('"') and tokk.endswith('"'):  # dealing with examples like "The_United_Kingdom_of_xxx"
                ...
                elif is_frame:  # 字典中没有这个relation/token
                ...
                if is_in_enc:
                    ...
                    bpe_toks = self._tok_bpe(tokk)  # 是free-form English
            bpe_tokens.append(bpe_toks)
            each_token_length.append(len(bpe_toks))
        bpe_tokens = [b for bb in bpe_tokens for b in bb]  # arg1-of 变成了两个token, 所有之前的alignment并不是最后sequence的最后下标, 但是对于找到最小alignment是足够的
        bpe_token_ids = [self.encoder.get(b, self.unk_token_id) for b in bpe_tokens]
        assert sum(each_token_length) == len(bpe_token_ids)
        
        return bpe_token_ids, {'each_token_length': each_token_length}
