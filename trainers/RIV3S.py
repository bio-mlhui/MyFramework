...
class Trainer_Referring_ImageVideo3D_Segmentation:
    def __init__(self, 
        ...
        self.iteration = -1
        if trainer_ckpt != '':
            self.load_ckpt(trainer_ckpt, resume=resume, strict_load=strict_load)
        else:
            logging.info('trainer的初始化没有用任何ckpt')
            print('trainer的初始化没有用任何ckpt')
        
        if (not resume) and self.is_main_process:
            self.save_ckpt(metrics=None)
...
@register_task
def rvos(configs, process_id, device, num_processes,):
    return Trainer_Referring_ImageVideo3D_Segmentation(
        process_id, device, num_processes,
        ...
        strict_load=configs['strict_load'],
        resume=configs['resume'],
    )

@register_task
def rios(configs, process_id, device, num_processes,):
    return Trainer_Referring_ImageVideo3D_Segmentation(
        process_id, device, num_processes,
        output_directory=configs['out_dir'],
        data_configs=configs['data'],
...
@register_task
def r3os(configs, process_id, device, num_processes,):
    return Trainer_Referring_ImageVideo3D_Segmentation(
        process_id, device, num_processes,
        output_directory=configs['out_dir'],
        data_configs=configs['data'],
