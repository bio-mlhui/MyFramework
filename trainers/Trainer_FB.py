...
class Trainer_FB:
    def __init__(self, configs):
        ...
        assert len(self.train_loaders) == 1
        ...
        self.iteration_per_epc = len(self.train_loaders[0].dataset) // configs['optim']['batch_sizes'][0]

        if type(self.ckpted_iters) == list and self.ckpted_iters[0] == 'epoch':
            self.ckpted_iters = int(self.ckpted_iters[1] * self.iteration_per_epc)
    ...
    def train(self):   
        ...
        for loader in self.train_loaders:
            for idx, batch_dict in enumerate(loader):
                ...
                batch_dict['num_epoch'] = self.num_iterations // self.iteration_per_epc
                iteration_time = time.time()

                model_without_ddp = self.model.module if isinstance(self.model, DDP) else self.model
                loss_dict_unscaled = model_without_ddp.forward_backward(batch_dict) 
