    ...
    def train(self):
        ...
        for self.epoch in range(self.epoch, self.total_epochs):
            ...
            for idx, bactch_dict in enumerate(self.trn_loader):
                ...
                # exist_queries, list[list[str], ni], b
                exist_queries = bactch_dict['exist_queries']
                # tensor(int) for a2ds
                ...
                with amp.autocast(enabled=configs.optimization.enable_amp):
                    diffusion_loss = self.model(samples, valid_indices, text_queries, targets, exist_queries)
