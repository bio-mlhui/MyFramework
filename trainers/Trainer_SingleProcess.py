...
class Trainer_SingleProcess:
        ...
        for epoch in range(self.epochs):
            for idx, batch_dict in enumerate(self.train_loader):
                ...
                self._log(loss_dict_unscaled=loss_dict_unscaled_item,
                        ...
                        lr_group_dicts=self.model.get_lr_group_dicts())
    ...
    def save_ckpt(self):
        ...
        if comm.is_main_process():
            ...
            checkpoint_dict = {
                ...
                'optimizer': self.model.optimize_state_dict(),
    ...
    def evaluate(self):
        ...
        evaluate_metrics = self.eval_function(model = self.model,  output_dir = self.iteration_dir)
        if comm.is_main_process():
    ...
    def log_header(self, iteration_time, sample_idxs):
        # 只计算一个epoch的估计时间
        one_epoch_iterations = len(self.train_loader.dataset) // len(sample_idxs)
